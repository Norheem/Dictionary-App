...
app.config['MYSQL_DATABASE_PASSWORD'] = 'nohimsulaiman'
...
def index():
    ...
    if request.method == 'POST':
        ...
        if user_input == '':
             flash('WARNING!!! \nPlease enter a word', 'flash_error')
        else:
            conn = mysql.get_db()
            cur = conn.cursor() 
            ...
            if(len(rv) > 0):
                ...
                user_response = 'WARNING!!! \nThe word cannot be found in this dictionary, please try again with another word'
...
def add_word():
     ...
     meaning = req['meaning']
     if word == '' or meaning == '':
         flash('WARNING!!! \nPlease fill in all fields, to add a new word', 'flash_error')
     else:     
        conn = mysql.get_db()
        cur = conn.cursor() 
...
def edit_word(id):
     ...
     meaning = req['meaning']
     if word == '' or meaning == '':
         flash('WARNING!!! \nPlease fill in all fields, to edit the word', 'flash_error')
     else:   
        conn = mysql.get_db()
        cur = conn.cursor() 
        cur.execute('update word set word=%s, meaning=%s where id=%s', (word,
        meaning, word_id))
        conn.commit()
        cur.close()
